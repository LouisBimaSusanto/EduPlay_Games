...
export function GameBatuLoncatan({ onComplete }) {
  ...
  return (
    <>
        ...
        <AnimatePresence>
          {Array.from({ length: roundData.targetTaps }).map((_, i) => (
            <motion.div
              ...
              className="text-[12rem] drop-shadow-[0_20px_20px_rgba(0,0,0,0.4)]"
            >
              🍃
            </motion.div>
          ))}
      ...
      {/* Top Area: Character */}
      <div className="w-full flex justify-start items-end flex-1 relative">
        <AnimatePresence mode="popLayout">
          <motion.div 
            ...
            className="absolute left-10 bottom-0 text-[16rem] drop-shadow-[0_20px_30px_rgba(0,0,0,0.6)] z-20"
          >
            {roundData.emoji}
          </motion.div>
        </AnimatePresence>
      ...
      <div className="w-full flex justify-center pb-12 z-20 relative shrink-0">
